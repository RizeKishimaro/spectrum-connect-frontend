...
export default function WebPhone() {
  ...
  // Add a useRef for the audio element at the top of the component with the other refs
    ...
    try {
      ...
      ua.on("newRTCSession", (data) => {
        ...
        session.on("ended", () => {
          ...
          // Clear the audio stream when call ends
          if (audioRef.current) {
  ...
  const makeCall = () => {
    ...
    try {
      const callSession = uaRef.current.call(phoneNumber, options)
  ...
  const toggleSpeaker = () => {
    ...
    if (audioRef.current && "setSinkId" in HTMLMediaElement.prototype) {
      try {
        // Only attempt to switch the output device if the audio is already playing
        if (!audioRef.current.paused) {
          // Switch between "default" and "speaker" output devices
          ...
          console.warn("Audio is not playing yet. Cannot switch audio output.")
        }
      } catch (error) {
        console.error("Error switching audio output:", error)
      }
    ...
    <div className="flex justify-center items-center min-h-screen bg-transparent p-4">
      <Card className="w-full max-w-lg shadow-lg">
        ...
        <audio
          ...
          playsInline
          className="hidden"
          // The volume is controlled by the speaker button
          volume={isSpeaker ? 1.0 : 0.7}
        />
      </Card>
    </div>
  )
